...
{
	return bpos.x + bpos.y * CHUNK_WIDTH
	       + bpos.z * CHUNK_HEIGHT * CHUNK_WIDTH;
}

BlockPosition blockpos_from_chunkpos(ChunkPosition cpos)
{
	return (BlockPosition){ .x = cpos.x * CHUNK_WIDTH,
				.y = cpos.y * CHUNK_HEIGHT,
				cpos.z = cpos.z * CHUNK_WIDTH };
}
...
{
	return (BlockPosition){ (u32)global.x % CHUNK_WIDTH,
				(u32)global.y % CHUNK_HEIGHT,
...
static inline void
determine_block(Chunk *chunk, BlockPosition bpos, u32 x, u32 y, u32 z)
{
	...
	float density
		= pow(stb_perlin_noise3(pick.x, pick.y, pick.z, 0, 0, 0), 2)
		  * 300;
...
void chunks_meshing_process(GameState *state, float delta)
{
	...
	FIU(world->load_chunk_positions_size) {
		...
		if ((atpos->value.flags & CHUNK_FLAG_MESH_CURRENT) != 0)
			continue;
		Frustum *frustum = &state->camera_frustum;
		if (!is_chunk_in_frustum(&atpos->value, frustum)) continue;

		if (chunk_mesh_mesh(&atpos->value, world)) break;
	}
}
